...
|---|---|---|
| `index.html` | **User** — คนทั่วไปที่จะใช้แอป | App Demo ที่กดเล่นได้เหมือนของจริง ภาษาคนทั่วไป ไม่มีศัพท์เทคนิค |
| `architecture.html` | **กรรมการ/นักพัฒนา** — คนที่อยากรู้ว่าโมเดลทำงานตรงไหนบ้าง | โครงสร้างระบบ พร้อมแผง "AI decision log" ที่โชว์สัญญาณ GNN / XGBoost / Rule-based / Shortfall Pressure ของทุกการโอน และปุ่มลัดทดสอบเคสเสี่ยงต่าง ๆ รวมถึงปุ่มจำลอง Shortfall Pressure เพื่อดูว่า Model 1 ส่งผลต่อ Model 2 จริงยังไง |
| `shortfall.html` | **กรรมการ/นักพัฒนา** | Monte Carlo 1,500 เส้นทาง คำนวณความน่าจะเป็นที่จะเงินขาดมือ (ต้นทางของ Shortfall Pressure ที่ใช้ใน `architecture.html`) |

`index.html` และ `architecture.html` ใช้ UI/สีเดียวกัน (โทรศัพท์จำลอง + panel ด้านขวา) เพื่อไม่ให้ผู้ดูสับสนว่าเป็นระบบคนละตัว แต่เนื้อหาฝั่งขวาต่างกันตามผู้ฟัง และทั้งสองไฟล์ใช้เกณฑ์ Risk Score เดียวกัน (Low &lt;0.25 / Medium 0.25–0.50 / High 0.50–0.75 / Critical ≥0.75) เพื่อไม่ให้ผลลัพธ์ขัดกันเอง
